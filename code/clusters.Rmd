...
```{r chisq-distribution}
  ...
  chisq.p1.gg <- chsqStatistics(data=bugs.tibl, pos=positions.group.1, time="pm")
  chisq.p2.gg <- chsqStatistics(data=bugs.tibl, pos=positions.group.2, time="pm")
  chisq.p3.gg <- chsqStatistics(data=bugs.tibl, pos=positions.group.3, time="pm")
...
chsqStatistics <- function(data, pos, time) {
...
# 
  ...
  stats.p1 <- chisq.test(x=control.vector, y=snh.vector)
  
  # http://ianmadd.github.io/pages/Confidence_Intervals_Part4.html
  # probability based on a quantile
  probabilityDensity.p1 <- pchisq(stats.p1$statistic, df = stats.p1$parameter)
  #      .9368 = pchisq(48.58, df = 35)
  degreesFreedom.p1 <- stats.p1$parameter
  # quantile based on a probability. (value of the x axis)
  chi95pct.p1 <- qchisq(.95, df=stats.p1$parameter)
  ...
  gg <- ggplot() +
      ...
      stat_function(fun = dchisq, args = list(df = degreesFreedom.p1)) +
      # https://stackoverflow.com/questions/48753007/using-stat-function-to-draw-partially-shaded-normal-curve-in-ggplot2                 
      stat_function(fun = dchisq, args = list(df = degreesFreedom.p1),
                   xlim = c(0, chi95pct.p1),
                  geom = "area", fill = "green", alpha = .4) +
    
    #geom_vline(xintercept = chi95pct) +
    geom_vline(xintercept = stats.p1$statistic) +
    expand_limits(x = c(0,75)) +
    geom_textvline(xintercept = chi95pct.p1, label = "probability density: 95%", 
                   hjust = 0.5, linetype = 2, color = "blue4") 
