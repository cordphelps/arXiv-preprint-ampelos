...
# 
  ...
  control.vector <- control.group.1.tibl %>% pull(crabSpiders)
  snh.vector <- SNH.group.1.tibl  %>% pull(crabSpiders)
  ...
  # quantile based on a probability.
  density95pct <- qchisq(.95, df=stats$parameter)
  ...
  ggplot(data.frame(x = c(0, 20)), aes(x = x)) +
    ...
    geom_vline(xintercept = density95pct) +
    geom_vline(xintercept = stats$statistic) +
    expand_limits(x = c(0,150)) 
  
  
  chisq.1.tbl <- as_tibble(countsControl=control.group.1.tibl$crabSpiders, countsSNH=SNH.group.1.tibl)
...
```{r mann-whitney-u-test}
  ...
  make-MWU-graphics(control= control.group.1.tibl, treatment=SNH.group.1.tibl)
...
make-MWU-graphics <- function(control, treatment) {

  # mirror https://stat-methods.com/home/mann-whitney-u-r/

  # append one tibble to the other
  merged.tibl <- dplyr::bind_rows(control, treatment) 
  ...
  ggBox <- ggplot() +
              ...
              geom_boxplot(control, mapping=aes(x = transect, y = crabSpiders, 
                            fill = 'red')) + 
              geom_boxplot(treatment, mapping=aes(x = transect, y = crabSpiders, 
                            fill = 'green')) + 
